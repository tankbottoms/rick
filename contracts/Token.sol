...
contract Token is IToken, ERC721, ReentrancyGuard, Ownable {
    ...
    function _getHTMLBase64(uint256 tokenId) internal view returns (string memory) {
        ...
        if (_ricked[tokenId] || ints[1] > 128 || true) {
            strings[
                4
            ] = '<g style="transform:translate(36px, 241px);cursor:pointer" onclick="startRickRoll()"> <rect width="36px" height="17.3333px" rx="8px" ry="8px" fill="rgba(0,0,0,0.6)"></rect><text id="playPauseButton" x="8px" y="11.333px" font-family="\'Courier New\', monospace" font-size="8px" fill="white">PLAY</text></g>';
