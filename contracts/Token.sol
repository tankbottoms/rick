...
contract Token is IToken, ERC721, ReentrancyGuard, Ownable {
    ...
    function _getHTMLBase64(uint256 tokenId) internal view returns (string memory) {
        string[1] memory merkaba = [string(assets.getAssetContentForId(4))];
        ...
        string memory uniswap = string(
            abi.encodePacked(
                ...
                clickMeText,
                '<g style="transform: scale(0.6);transform-origin: center;"><g style="/*animation: spin 25s linear infinite;*/transform-origin:center;color: #fff',
                '">',
